...
async function processDocument(bucketName, file) {
    ...
    if(docData.page.length > 0) {
        try {
            ...
            console.error('Error saving entities document to BigQuery');
            e.forEach(err => {
                err.errors.forEach(error => {
                    console.error(`Column: ${error.location} - ${error.message}`);
                });
            });
        }
    }
    ...
    if(lapdoc.docFound) {
        try {
            ...
            console.error('Error saving lap document to BigQuery');
            e.forEach(err => {
                err.errors.forEach(error => {
                    console.error(`Column: ${error.location} - ${error.message}`);
                });
...
processDocument('coe-forms', 'just_the_table.pdf');
