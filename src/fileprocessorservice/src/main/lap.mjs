...
class LapDoc extends DocInterface {
    ...
    /**
     * Checkes if we have processed all the applicants columns in the table.
     * If so (6 columns), the add the applicant to the docRecord and reset
     * the applicant array
     */
    applicantEntry() {
    ...
    /**
     * Set/Reset the applications array
     */
    _newApplicants() {
        this.applicants = {
            first_name: null,
    ...
    /**
     * Convert the no/yes to boolean value
     * @param {string} value The value to check, should be yes or no
     * @return {boolean}
     */
    _convertNoYes(value) {
    ...
    /**
     * Convert a string to a string data type for BigQuery
     * @param {string} value
     * @return {string}
     */
    _convertDate(value) {
